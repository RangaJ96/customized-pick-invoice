...
class ObjectTree:	
	...
	def connect(self, plot=False, export_df=False):
		...
		#print(f'\n print the df head : {df.head()}\n\n')

		#ob_df = df.iloc[nearest_dest_ids_vert]
		#print(ob_df['Object'])


		try:
			#df['below_object'] = df.loc[nearest_dest_ids_vert, 'Object'].values
			#print(df['below_object'])
			#ob_df = df.iloc[nearest_dest_ids_vert]
			#ob_df_2 = ob_df['Object']
			#df['below_object'] = ob_df_2

			ob_df = df.iloc[nearest_dest_ids_vert]
			ob_df_2 = ob_df['Object']
			below_object_list = ob_df_2.values.tolist()
			df['below_object'] = below_object_list

		except:
			print("Remove empty plotting")
		...
		try:
			...
			ob_side = df.iloc[nearest_dest_ids_hori]
			ob_side_2 = ob_side['Object']
			side_object_list = ob_side_2.values.tolist()
			df['side_object'] = side_object_list
		except:
			print("Remove empty plotting")
...
class Graph:
	...
	def make_graph_data_event(self, graph_dict, text_list):
		'''
		...
		try:
			...
			print("Index can be exceeded")


if __name__ == "__main__":
	print(os.getcwd())
	df = pd.read_csv('.\data\object_map_1.csv')

	path = r'.\data\deskew.jpg'
	...
	graph.make_graph_data_event(graph_dict, text_list)
	#print(A)
	#print('-----------------------------------------------------------------\n')
	#print(X)